...
#if _TAREADOR_
	  tareador_end_task();
	  tareador_start_task("multisort-3");
#endif

	  multisort(n/4L, &data[n/2L], &tmp[n/2L]);
#if _TAREADOR_
	  tareador_end_task();
	  tareador_start_task("multisort-4");
#endif
	  multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L]);
#if _TAREADOR_
	  tareador_end_task();
	  tareador_start_task("merge-4-1");
#endif
	  merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L);
#if _TAREADOR_
	  tareador_end_task();
	  tareador_start_task("merge-4-2");
#endif
	  merge(n/4L, &data[n/2L], &data[3L*n/4L], &tmp[n/2L], 0, n/2L);
#if _TAREADOR_
	  tareador_end_task();
	  tareador_start_task("merge-2-1");
#endif
